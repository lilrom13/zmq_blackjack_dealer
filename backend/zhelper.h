...
static auto
s_send (zmq::socket_t *socket, std::string string, zmq::send_flags flags = zmq::send_flags::none) {
    auto res = socket->send(zmq::message_t(string), flags);
    return res;
}
...
s_sendmore (zmq::socket_t *socket, std::string string) {
    auto res = socket->send(zmq::message_t(string), zmq::send_flags::sndmore);
    return res;
}
